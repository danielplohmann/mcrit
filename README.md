 ...
 * 2026-08-24 v1.7.0:  Database schema change: disassembly (`_xcfg`) now lives in dedicated `xcfg`/`query_xcfg` collections keyed by function id, instead of inline in every function document (#137). On the reference corpus (11.6M functions) this shrinks the functions collection from 18,353 MB to 3,130 MB and reduces bytes read per matching job accordingly; serving is correct before, during and after the migration because every reader falls back to an inline `_xcfg` when no blob document exists - so upgrading does not require an immediate migration window. To complete the split (and actually reclaim the space), run the bundled migration: `python -m mcrit.migrations.migrate_xcfg_split --mode copy --target <rehearsal_db>` for a read-only rehearsal with byte-for-byte verification, then `--mode inplace` (resumable, writes blobs before unsetting them), `--mode verify` and `--mode unsplit` for rollback; `/status` reports `inline_xcfg_remaining` so a half-migrated instance announces itself instead of quietly staying on the old shape. With the blobs split out, `STORAGE_DROP_DISASSEMBLY` now genuinely reclaims space by dropping the collection where blanking in place did not. Minhash exports and `/status` additionally record which smda escaper produced their minhashes as a per-architecture fingerprint (#142): since minhashes are derived from smda's escaped instruction representation, an escaper change (e.g. smda 4.4.5 reclassifying segment-qualified memory operands, which silently altered ~20% of stored minhashes on a real corpus) previously made imported signatures silently under-report matches; imports now warn per architecture on escaper mismatch instead.
