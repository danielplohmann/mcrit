...
def run_one(sample_id, flags_on, batch=None, timeout_seconds=3600, tag="ov", k=None, mem_limit_mib=16384, cache_max_entries=6000000):
    ...
    command = [
        ...
        SCRATCH,
        "--tag",
        "%s_%s" % (tag, "on" if flags_on else "off"),
    ]
    if k is not None:
        command += ["--k", str(k)]
    if flags_on:
        ...
        command += ["--cand-accum", "numpy", "--vectorized", "1", "--persist-cache", "1", "--cache-max-entries", str(cache_max_entries), "--fetch-threads", "8"]
    else:
        command += ["--cand-accum", "dict", "--vectorized", "0", "--persist-cache", "0", "--fetch-threads", "1", "--fetch-slice", "500000"]
    if batch is not None:
        command += ["--batch", str(batch)]

    started = time.perf_counter()
    try:
        completed = subprocess.run(command, capture_output=True, text=True, timeout=timeout_seconds, preexec_fn=_limit_address_space(mem_limit_mib))
    except subprocess.TimeoutExpired:
        return {"status": "timeout", "seconds": round(time.perf_counter() - started, 1)}
    if completed.returncode != 0:
        return {"status": "error", "returncode": completed.returncode, "stderr_tail": completed.stderr[-800:], "seconds": round(time.perf_counter() - started, 1)}
...
def write_summary():
    try:
        ...
        log("WARNING: write_summary failed (%s: %s) - results are safe in the JSONL" % (type(error).__name__, error))
...
def _write_summary():
    ...
    if mismatched:
        ...
        for r in mismatched:
            lines.append("| %d | %s | %s | `%s` | `%s` |" % (r["sample_id"], r["num_functions"], r["off"].get("k"), r["off"]["digest"], r["on"]["digest"]))
    if broken:
        lines.append("\n### Incomplete runs\n")
        lines.append("| sample | fns | off | on |\n|---|---|---|---|")
        for r in broken:
            lines.append("| %d | %s | %s | %s |" % (r["sample_id"], r["num_functions"], r["off"].get("status"), r["on"].get("status")))
    if ok:
        ...
        if slower:
            ...
            for r in sorted(slower, key=lambda x: x["on"]["wall_seconds"] / max(x["off"]["wall_seconds"], 1e-6))[-15:]:
                lines.append(
                    "| %d | %s | %.1f | %.1f | %s | %s |"
                    % (r["sample_id"], r["num_functions"], r["off"]["wall_seconds"], r["on"]["wall_seconds"], r["off"]["peak_mib"], r["on"]["peak_mib"])
                )
        ...
        for delta, r in peaks[:10]:
            lines.append("| %d | %s | %s | %s | +%s |" % (r["sample_id"], r["num_functions"], r["off"]["peak_mib"], r["on"]["peak_mib"], delta))

    if batch:
        lines.append("\n## Phase B - batch size with the flags on\n")
        lines.append("Does fix 03's O(candidates) scoring memory make BUG-3's shipped default of 10000 safe again?\n")
        lines.append("| sample | batch | wall s | peak MiB | digest | matches b=200 |")
        lines.append("|---|---|---|---|---|---|")
        baseline = {}
        for r in batch:
            if r["result"].get("status") != "ok":
                lines.append("| %d | %s | - | - | %s | - |" % (r["sample_id"], r["batch"], r["result"].get("status")))
                continue
            result = r["result"]
            if r["batch"] == 200:
            ...
            lines.append("| %d | %s | %.1f | %s | `%s` | %s |" % (r["sample_id"], r["batch"], result["wall_seconds"], result["peak_mib"], result["digest"][:16], same))
    ...
    if evict:
        ...
        for r in evict:
            ...
            lines.append(
                "| %d | %s | %s | %s | `%s` | %s |"
                % (r["sample_id"], res.get("status"), res.get("wall_seconds", "-"), res.get("peak_mib", "-"), res.get("digest", "-"), r.get("verdict"))
            )
...
def main():
    ...
    log("budget %.1f h, deadline %s UTC" % (args.budget_hours, datetime.fromtimestamp(deadline, timezone.utc).strftime("%H:%M:%S")))
    ...
    if not args.skip_phase_a:
        ...
        for index, (sample_id, num_functions, bucket) in enumerate(selection):
            ...
            for flags_on in order:
                outcome["on" if flags_on else "off"] = run_one(sample_id, flags_on, timeout_seconds=args.timeout_seconds)
            row = {
                "phase": "A",
                "sample_id": sample_id,
            ...
            if outcome["off"].get("status") == "ok" and outcome["on"].get("status") == "ok":
                ...
                log(
                    "A %3d/%d sample %5d (%5s fns) %6.1fs -> %6.1fs  %.2fx  %s"
                    % (
                        index + 1,
                        len(selection),
                        sample_id,
                        num_functions,
                        outcome["off"]["wall_seconds"],
                        outcome["on"]["wall_seconds"],
                        outcome["off"]["wall_seconds"] / max(outcome["on"]["wall_seconds"], 1e-6),
                        verdict,
                    )
                )
            else:
                log("A %3d/%d sample %5d (%5s fns) off=%s on=%s" % (index + 1, len(selection), sample_id, num_functions, outcome["off"].get("status"), outcome["on"].get("status")))
    ...
    for sample_id, sample_k in ((5188, 2), (8263, 1)):
        ...
        for batch in (200, 1000, 5000, 10000):
            if time.time() > deadline:
                ...
                out_of_budget = True
                break
            result = run_one(sample_id, True, batch=batch, k=sample_k, timeout_seconds=args.timeout_seconds, tag="ovb", mem_limit_mib=12288)
            append_row({"phase": "B", "sample_id": sample_id, "batch": batch, "k": sample_k, "result": result, "utc": datetime.now(timezone.utc).isoformat()})
            write_summary()
            log(
                "B sample %d k=%d batch %5d -> %s %s"
                % (
                    sample_id,
                    sample_k,
                    batch,
                    result.get("status"),
                    ("%.1fs / %s MiB" % (result.get("wall_seconds", 0), result.get("peak_mib"))) if result.get("status") == "ok" else "",
                )
            )
    ...
    KNOWN_STOCK_DIGESTS = {8263: "335875610c7491258cbcc2a5da492133", 601: "f3c4aebadf8b9be7af56306b71f2c7a8"}
    for sample_id, sample_k in ((601, 1), (8263, 1)):
        if time.time() > deadline:
            log("budget exhausted, stopping phase C")
            break
        result = run_one(sample_id, True, k=sample_k, timeout_seconds=args.timeout_seconds, tag="ovc", cache_max_entries=2000000)
