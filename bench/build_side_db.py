...
def main():
    ...
    cursor = src.functions.find({}, {"_id": 0, "function_id": 1, "sample_id": 1, "minhash": 1}, no_cursor_timeout=True).batch_size(args.batch)
    ...
    try:
        for doc in cursor:
            ...
            if len(hex_batch) >= args.batch:
                ...
                print("  %s / %s  (%.1f%%)  %.0f docs/s  %.0fs elapsed" % ("{:,}".format(n), "{:,}".format(total), 100.0 * n / total, n / elapsed, elapsed), flush=True)
    finally:
        cursor.close()
    ...
    print("copied %s documents (%s without a minhash) in %.0fs" % ("{:,}".format(n), "{:,}".format(n_no_minhash), time.perf_counter() - t0))
    ...
    for name in ("fn_hex", "fn_bin"):
        ...
        print(
            "%-8s count=%-12s size=%8.2f GB  storage=%8.2f GB  index=%6.2f GB  avgObj=%d B"
            % (name, "{:,}".format(s["count"]), s["size"] / 1e9, s["storageSize"] / 1e9, s["totalIndexSize"] / 1e9, s.get("avgObjSize", 0))
        )
    s = src.command("collstats", "functions")
    print(
        "%-8s count=%-12s size=%8.2f GB  storage=%8.2f GB  index=%6.2f GB  avgObj=%d B"
        % ("functions", "{:,}".format(s["count"]), s["size"] / 1e9, s["storageSize"] / 1e9, s["totalIndexSize"] / 1e9, s.get("avgObjSize", 0))
    )
    return 0
