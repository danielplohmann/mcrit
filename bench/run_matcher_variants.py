...
class WorkerStub:
    ...
    def calculateMinHashes(self, function_entries):
        ...
        for function_entry in function_entries:
            ...
            smda_functions.append(
                (function_entry.function_id, SmdaFunction.fromDict(function_entry.xcfg, binary_info=binary_info)))
        smda_functions = [
            (function_id, smda_function) for function_id, smda_function in smda_functions
...
def main():
    ...
    print("%-28s flags=%d  %8.2fs  digest %s (len %d, %d query ids canonicalized)" % (
        label, opts.flags, wall, sha, length, n_query_ids))
    return 0
