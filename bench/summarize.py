...
def main():
    ...
    print(hdr % ("family", "sample", "k/b", "pool", "wall_s", "peak_MiB", "self_MiB", "pairs", "cached_fns", "mongo_MB", "mongo_s", "calls"))
    ...
    print("%-14s %-6s %-6s %8s  " % ("family", "sample", "pool", "wall_s") + " ".join("%18s" % p[:18] for p in phase_names))
    for r in sorted(rows, key=key):
        ...
        print("%-14s %-6s %-6s %8.1f  " % ((m.get("family") or "?")[:14], m["sample_id"], "pool" if c["pool_matching"] else "single", m["wall_seconds"]) + " ".join(cells))
    ...
    for site, a in sorted(agg.items(), key=lambda x: -x[1]["bytes"])[:14]:
        print("%-52s %10s %12.1f %10.1f %6d" % (site[:52], "{:,}".format(a["calls"]), a["bytes"] / 1e6, a["read_seconds"], a["runs"]))
    ...
    for r in sorted(rows, key=key):
        ...
        if not r["config"]["pool_matching"] and mt["score_seconds"]:
            print(
                "  %-14s s%-6s %10s pairs in %7.2fs = %10s pairs/s"
                % (
                    (r["meta"].get("family") or "?")[:14],
                    r["meta"]["sample_id"],
                    "{:,}".format(mt["score_pairs"]),
                    mt["score_seconds"],
    ...
    for r in sorted(rows, key=key):
        ...
        print(
            "  %-14s s%-6s decode total %6.2fs (%4.1f%% of wall)  %s"
            % (
                (r["meta"].get("family") or "?")[:14],
                r["meta"]["sample_id"],
                tot,
