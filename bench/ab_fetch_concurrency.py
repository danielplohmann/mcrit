...
def main():
    ...
    print("fetching %s function_ids in slices of %s, threads %s, %d reps" % ("{:,}".format(len(all_ids)), "{:,}".format(args.slice), thread_counts, args.reps))
    ...
    for threads in order:
        ...
        print("  threads %2d  %8.3f s  %s docs  %s docs/s" % (threads, elapsed, "{:,}".format(found), "{:,}".format(int(found / elapsed))))
    ...
    for threads in thread_counts:
        ...
        summary.append({"threads": threads, "best_seconds": fastest, "speedup_vs_%d" % thread_counts[0]: round(baseline / fastest, 2)})
    ...
    with open(path, "w") as fh:
        json.dump({"ids": args.ids, "slice": args.slice, "reps": args.reps, "identical": len(checksums) == 1, "observations": observations, "summary": summary}, fh, indent=2)
    try:
        st = os.stat(args.out)
