...
def main():
    ...
    parser.add_argument("--persist-cache", type=int, default=None, help="1=retain MatchingCache across batches, 0=rebuild per batch (default: live config)")
    parser.add_argument("--cache-max-entries", type=int, default=None)
    parser.add_argument("--cand-accum", choices=["dict", "numpy"], default=None, help="candidate accumulation in getCandidatesForMinHashes (default: live config)")
    ...
    parser.add_argument("--hot-collection", default=None, help='C1: "db.collection" with slim binary minhash docs, e.g. mcrit_bench.fn_bin; "" = off')
    parser.add_argument(
        "--malloc-trim",
        type=int,
        default=0,
        help="1 = call libc malloc_trim(0) at each batch start and log the RSS it frees; distinguishes glibc arena fragmentation (RSS drops) from genuine retention (it does not)",
    )
    ...
    if args.malloc_trim:
        ...
        def _trimming_create_cache(self, candidate_groups):
            ...
            print("MALLOC_TRIM batch_boundary rss_before=%d kB rss_after=%d kB freed=%d kB" % (rss_before, rss_after, rss_before - rss_after), flush=True)
    ...
    name = name.replace(
        ".json",
        "_%s%s.json"
        % (getattr(config.STORAGE_CONFIG, "STORAGE_CANDIDATE_ACCUMULATION", "dict"), "_vec" if getattr(config.MINHASH_CONFIG, "MINHASH_MATCHING_VECTORIZED", False) else ""),
    )
    ...
    print("wrote %s" % path)
    print(
        "  wall %.1fs | container peak %.0f MiB | pairs %s | mongo %s calls / %.1f MiB / %.1fs"
        % (
            wall_seconds,
