...
def main():
    ...
    for batch_index, start in enumerate(starts):
        fid_to_minhash = {
            fe.function_id: fe.getMinHash(minhash_bits=bits)
            for fe in function_entries[start:start + batch_size]
        }
        ...
        print("  batch %3d  %8.3fs  pairs %12s  rss %6.0f -> %6.0f MiB  %s" % (
            batch_index, elapsed, "{:,}".format(pairs),
            rss_before_kb / 1024, rss_after_kb / 1024, digest))
    ...
    path = os.path.join(args.out, "cand_%s_s%d_k%d_b%d_%s.json" % (
        args.tag, args.sample_id, k, batch_size, args.accum))
    ...
    print()
    print("accum=%-5s  batches=%d  retrieval %.2fs  pairs %s  peak RSS %.0f MiB  digest %s" % (
        args.accum, len(starts), total_seconds, "{:,}".format(result["total_pairs"]),
        peak_rss_mib, result["digest_all_batches"]))
    print("wrote %s" % path)
