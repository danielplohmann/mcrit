...
def _fetch_chunk(collection, chunk):
    ...
    for doc in collection.find({"function_id": {"$in": chunk}}, PROJECTION):
        ...
        decoded.append((doc["function_id"], doc["sample_id"],
                        bytes.fromhex(mh) if isinstance(mh, str) else bytes(mh)))
    return decoded
...
def main():
    ...
    for rep in range(args.repeats):
        ...
        rows.append(row)
        print("%-10s t%-2d %-5s  %8.3fs  %7.1f MB  %6.1f B/doc  %s docs/s" % (
            args.layout, args.threads, row["state"], elapsed, wire / 1e6, row["bytes_per_doc"],
            "{:,}".format(row["docs_per_second"])), flush=True)
