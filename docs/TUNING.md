...
* **Pair budget** (`MINHASH_MATCHING_MAX_PAIRS`, default 50,000,000, new in v1.6.2) caps how many
  ...
  batch size above remains an upper bound on query functions per batch, so both limits apply and
...
* `STORAGE_CANDIDATE_ACCUMULATION`, `MINHASH_MATCHING_VECTORIZED`, `STORAGE_CACHE_FETCH_THREADS`
  and `STORAGE_MATCHING_CACHE_PERSIST` shipped in v1.6.0 and became the defaults in v1.6.1, so the
  table above describes stock behaviour from v1.6.1 onwards. On v1.5.x only the batch size,
  `BAND_MATCHES_REQUIRED` and the mongod cache size apply.
